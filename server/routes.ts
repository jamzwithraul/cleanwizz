...
export async function registerRoutes(_httpServer: Server, app: Express) {
  ...
  // Auth: protected by requireAuth (Supabase JWT). The harrietscleaning.ca/admin
  // SPA logs in via Supabase and sends Authorization: Bearer <access_token>.
  // The legacy quotes.harrietscleaning.ca SPA does not have login UI; it should
  // not be used to manage availability.
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /api/admin/test-transfer — TEMPORARY: validate Stripe Connect end-to-end
  //
  // The owner-bypass in the complete-job Edge Function means owner-completed
  // jobs never call stripe.transfers.create. To prove the Connect transfer
  // pipe works against a live destination account before any non-owner
  // contractor takes a real job, this endpoint fires a one-off transfer.
  //
  // Hard-gated:
  //   - requireAuth (Supabase JWT)
  //   - email allowlist (owner only)
  //   - amount capped at 200 cents (CAD $2.00)
  //   - destination must be a Connect account id (acct_*)
  //   - metadata.test_transfer is always set to "true" so payouts can be
  //     filtered out of any analytics later.
  //
  // Remove this endpoint after the test transfer is verified in Stripe.
  // ─────────────────────────────────────────────────────────────────────────
  const TEST_TRANSFER_OWNERS = new Set([
    "jamzwithraul@gmail.com",
    "admin@harrietscleaning.ca",
    "magic@harrietscleaning.ca",
  ]);
  app.post("/api/admin/test-transfer", requireAuth, async (req, res) => {
    try {
      if (!stripe) return res.status(503).json({ error: "Stripe not configured." });
      const email = (req.user?.email || "").toLowerCase();
      if (!TEST_TRANSFER_OWNERS.has(email)) {
      ...
      const transfer = await stripe.transfers.create({
        ...
        currency: "cad",
        destination,
        description: "Connect transfer test (one-off)",
        metadata: {
          test_transfer: "true",
          triggered_by: email,
      ...
      const stripeMsg = err?.raw?.message || err?.message || "Transfer failed.";
      console.error("[admin/test-transfer] failed:", stripeMsg);
